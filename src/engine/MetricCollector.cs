...
namespace Keymon
{
    ...
    public class MetricCollector
    {
        ...
        // object? sender: 이벤트를 발생시킨 객체 (여기서는 TaskPoolGlobalHook)
        // KeyboardHookEventArgs e: 어떤 키가 눌렸는지 정보를 담고 있음
        private void OnKeyPressed(object? sender, KeyboardHookEventArgs e)
        {
            DateTime now = DateTime.Now;
            var keyCode = e.Data.KeyCode;

            // lock(_keyTimes): _keyTimes를 기준으로 이 블록을 동기화합니다.
            // 이 블록이 실행 중일 때는 다른 스레드가 _keyTimes에 접근 불가.
            lock (_keyTimes)
            {
        ...
        private void OnMouseMoved(object? sender, MouseHookEventArgs e)
        {
            ...
            // Math.Sqrt: 제곱근. 피타고라스 정리로 이동 거리를 계산합니다.
            double distance = Math.Sqrt(dx * dx + dy * dy);

            if (distance > 100)
            {
                double timeElapsed = (now - _lastMouseTime).TotalSeconds;
                // 속도가 초당 1000픽셀 이상인 빠른 이동만 분석합니다.
                if (timeElapsed > 0 && (distance / timeElapsed) > 1000)
                {
                    // Math.Atan2: 두 점 사이의 각도를 라디안으로 반환. 여기서 도(degree)로 변환.
                    ...
                    if (_lastMouseAngle != -1000)
                    {
                        ...
                        // 각도 차이가 180도를 넘으면, 반대방향(360-angleDiff)으로 보정
                        if (angleDiff > 180) angleDiff = 360 - angleDiff;

                        // 135도 이상 방향 전환 = 급격한 꺾임 후보
                        if (angleDiff > 135)
                        {
        ...
        // 이 덕분에 Count가 항상 '최근 60초 내 횟수'를 나타냅니다.
